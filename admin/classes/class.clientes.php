		...
		try{
            ...
            if($nome=="")	{
                ...
                if ($clienteid <= 0) {
                    ...
                    $stmt = $this->conn->prepare("INSERT INTO clientes(nome,email,endereco,senha,telefone,hash,dtnascimento,observacao,empresaid) VALUES(:nome, :email, :endereco, :senha, :telefone, :hash, :dtnascimento, :observacao, :empresaid)");
                    $stmt->bindparam(":nome", $nome);
                    $stmt->bindparam(":email", $email);
                    $stmt->bindparam(":endereco", $endereco);
                    $stmt->bindparam(":senha", $new_password);
                    ...
                    $stmt = $this->conn->prepare("UPDATE clientes SET nome = :nome ,email = :email, endereco = :endereco ,senha = :senha , telefone = :telefone , dtnascimento = :dtnascimento , observacao = :observacao , hash = '', confirmada = 'S' WHERE clienteid = :clienteid");
                    $stmt->bindparam(":nome", $nome);
                    $stmt->bindparam(":email", $email);
                    $stmt->bindparam(":endereco", $endereco);
                    $stmt->bindparam(":senha", $new_password);
