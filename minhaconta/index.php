<?php
    ...
    $stmt = $auth_user->runQuery("SELECT count(*) as contador FROM agendamentos WHERE cancelado = 'N' AND clienteid = :clienteid and datainicio BETWEEN '".$dtinicio."' AND '".$dtfim."'");
    $stmt->execute(array(':clienteid'=>$_SESSION['user_session']));
    $row = $stmt->fetch();
    $numhoje = $row['contador'];


    $mes = date('m');
    $stmt = $auth_user->runQuery("SELECT count(*) as contador FROM agendamentos WHERE cancelado = 'N' AND clienteid = :clienteid AND MONTH(datainicio) = :mes ");
    $stmt->bindparam(':clienteid',$_SESSION['user_session']);
					...
					<!-- //header-ends -->
						<div class="outter-wp">
                            ...
                            <script type="text/javascript">
                                ...
                                var campos = 'agendamentoid, agendamentos.datainicio as data, empresas.nomefantasia as estabelecimento, profissionais.nome as profissional';
                                var alias = 'cliente';
                                var inner = '';
                                ...
                                var consulta = 'consultaI';
                                        ...
                                        <br><h1>Últimos estabelecimentos</h1>
                            ...
                            <div class="panel panel-primary">
                                ...
                                <div class="panel-body">
                                    <div class="list-group">
                                        ...
                                        if($_SESSION['ultimos'] != '') {
                                            $stmt = $auth_user->runQuery("SELECT pasta, nomefantasia FROM empresas WHERE excluido = 'N' and bloqueio = 'N' AND empresaid in (" . $_SESSION['ultimos'] . ") ");
                                            $stmt->execute();
