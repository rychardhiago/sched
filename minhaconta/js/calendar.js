...
(function($) {
	...
	Calendar.prototype._calculate_hour_minutes = function(data) {
		...
		$.each(data.events, function(k, e) {
            var s = moment(e.startD,'YYYY-MM-DD HH:mm:ss').toDate();
			var f = moment(e.endD,'YYYY-MM-DD HH:mm:ss').toDate();
			e.start = s.getTime();
			e.end = f.getTime();
		...
		switch($.type(source)) {
			...
			case 'string':
				if(source.length) {
					loader = function() {
						var events = [];
						var params = {from: self.options.position.start.getTime(), to: self.options.position.end.getTime(), profissionaiid: $('#profissionaiid').val()};
