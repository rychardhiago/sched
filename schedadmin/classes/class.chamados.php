...
class chamados{
	...
	public function register($valor){
		try{
            ...
            $tipo = $valor["tipo"];
            $situacao = (isset($valor["situacao"]) ? $valor["situacao"] : "A");
            $chamadoid = $valor["chamadoid"];
            $addmensagem = $valor['addmensagem'];
            $identificador = date("dmY_Hi");

            if($chamadoid > 0){
                $stmt = $this->conn->prepare("UPDATE chamados set mensagem = CONCAT(mensagem,:mensagem), tipo = :tipo, situacao = :situacao, assunto = :assunto  WHERE chamadoid = :chamadoid");
